...
def get_route_osrm(lat1, lon1, lat2, lon2, profile='driving'):
    ...
    if resp.status_code == 200:
        ...
        return [[lat, lon] for lon, lat in coords], distance, duration
    else:
        print(f"OSRM Error: Status {resp.status_code}, Response: {resp.text}")
        return [[lat1, lon1], [lat2, lon2]], 0, 0
...
def generate_map(from_place, to_place, mode='car', output_dir=r'F:\Full_Stack_Projects\Smart-Navigation\client\public'):
    excel_file = r'F:\Full_Stack_Projects\Smart-Navigation\python_service\tn_enhanced_safety_analysis_20250915_122616.xlsx'
    geojson_file = r'F:\Full_Stack_Projects\Smart-Navigation\python_service\TAMIL NADU_ASSEMBLY.geojson'
    ...
    route_coords, distance, duration = get_route_osrm(from_lat, from_lon, to_lat, to_lon, profile)
