...
const PoliceLocationHistory = ({ selectedUser }) => {
    ...
    /* ── Live clock ── */
    useEffect(() => {
        const tick = () => setClock(new Date().toLocaleTimeString("en-IN", { hour12: true, timeZone: "Asia/Kolkata" }) + " IST");
        tick();
    ...
    useEffect(() => {
        ...
        (async () => {
            try {
                ...
                let hist = rawData.map(h => ({
                    ...h,
                    lat: parseFloat(h.latitude ?? h.lat),
                    lng: parseFloat(h.longitude ?? h.lng)
                }));
                
                // Fallback to simulated data if no history exists (for demo purposes)
                if (hist.length === 0) {
                    const baseLat = 13.0827; // Center around Chennai
                    ...
                    for (let i = 0; i < 8; i++) {
                        hist.push({
                            ...
                            recorded_at: new Date(now.getTime() - i * 3600000).toISOString() // 1 hour intervals
                        });
                    }
                }
                
                // Fetch location names for the first 12 pings (to avoid rate limits/performance issues)
                const histWithNames = await Promise.all(hist.map(async (h, i) => {
                    if (i < 12 && h.lat && h.lng) {
                        try {
                            const detailRes = await fetch(`https://nominatim.openstreetmap.org/reverse?format=json&lat=${h.lat}&lon=${h.lng}&zoom=18&addressdetails=1`);
                            if (!detailRes.ok) throw new Error("Fetch failed");
                            const detailData = await detailRes.json();
                            const displayName = detailData.display_name || "";
                            const parts = displayName.split(',');
                            const name = parts.length >= 2 ? `${parts[0].trim()}, ${parts[1].trim()}` : (displayName || "Point of Interest");
                            return { ...h, location_name: name };
                        } catch (e) {
                            return { ...h, location_name: "Simulated Location Pin" };
                        }
                    }
                    return h;
                }));
                
                setHistory(histWithNames);
            } catch (e) {
    ...
    useEffect(() => {
        ...
        setStats({
            ...
            latest: history[0] ? new Date(history[0].recorded_at).toLocaleString("en-IN", { hour12: true }) : "N/A",
            earliest: history[history.length - 1] ? new Date(history[history.length - 1].recorded_at).toLocaleString("en-IN", { hour12: true }) : "N/A",
        });
    }, [history]);
        ...
        <div className="history-root">
            ...
            <div className="hist-cmd-bar">
                <div className="hist-brand">
                    ...
                    <div>
                        ...
                        <p>📍 Travel History · {selectedUser?.name || "No Subject"} · SMARTNAV OPS</p>
                    </div>
                </div>
            ...
            <div className="history-body">
                ...
                <div className="hist-left">
                    ...
                    {stats && (
                        <div className="hist-card">
                            <div className="hist-card-title"><span className="hist-dot" style={{ background: "#00ff9d", boxShadow: "0 0 6px #00ff9d" }} />LOCATION INTELLIGENCE</div>
                    ...
                    {history.length > 0 && (
                        <div className="hist-card">
                            <div className="hist-card-title"><span className="hist-dot" style={{ background: "#ff3700", boxShadow: "0 0 6px #ff3700" }} />TRAVEL HISTORY</div>
                            <div className="ping-list">
                                {history.slice(0, 12).map((loc, i) => (
                                    <div key={i} className="ping-item" onClick={() => { if (leafletMap.current) leafletMap.current.flyTo([loc.lat, loc.lng], 16, { duration: 1 }); }}>
                                        ...
                                        <div style={{ flex: 1 }}>
                                            <div className="ping-coords" style={{ fontWeight: 'bold' }}>📍 {loc.location_name || "Triangulating Place..."}</div>
                                            <div className="ping-coords" style={{ fontSize: '10px', opacity: 0.7 }}>{loc.lat.toFixed(5)}°N, {loc.lng.toFixed(5)}°E</div>
                                            <div className="ping-time">{new Date(loc.recorded_at).toLocaleString("en-IN", { hour12: true })}</div>
                                        </div>
                                    </div>
                                ))}
                            </div>
                ...
                <div className="hist-map-center">
                    ...
                    <div className="map-overlay-badge">
                        📍 TRAVEL HISTORY · {history.length} PINGS · {selectedUser?.name || "SUBJECT"}
                    </div>
                </div>
            </div>
        </div>
