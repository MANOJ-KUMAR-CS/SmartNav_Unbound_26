...
export default function PoliceMapView({ selectedUser }) {
    ...
    useEffect(() => {
        ...
        const C = window.Cesium;
        if (selectedUser) {
            let lat = selectedUser.lat;
            let lng = selectedUser.lng;
            let display = `Last Seen: ${selectedUser.name || "Subject"}`;
            
            // Fallback for missing coordinates (Simulated Data)
            if (!lat || !lng) {
                lat = 13.0827; // Default to Chennai
                lng = 80.2707;
                display = `Estimated Location (Simulated): ${selectedUser.name || "Subject"}`;
            }

            const result = {
                name: selectedUser.name || "Subject",
                lat: parseFloat(lat),
                lng: parseFloat(lng),
                display: display
            };
            setSearchRes(result);
            setTimeout(() => flyToPlace(result), 1000);
        }
    }, [mapReady, selectedUser, flyToPlace]);
